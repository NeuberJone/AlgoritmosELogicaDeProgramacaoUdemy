import java.util.Locale; // Biblioteca que o Locale pertence
import java.util.Scanner; // Biblioteca que o Scanner pertence

public class entradaDeDados {
    public static void main(String[] args) {

        Locale.setDefault(Locale.US); // Define o separador dos valores float como ponto e não virgula
        Scanner sc = new Scanner(System.in); // Objeto que permite entrada de dados

        double salario1, salario2;
        String nome1, nome2;
        int idade;
        char sexo;

        System.out.print("Nome da primeira pessoa: ");
        nome1 = sc.nextLine(); // Lê o texto até a quebra de linha

        System.out.print("Salario da primeira pessoa: ");
        salario1 = sc.nextDouble(); // Entrada de variável de ponto flutuante

        System.out.print("Nome da segunda pessoa: ");
        sc.nextLine(); // Faz a limpeza de buffer
        nome2 = sc.nextLine();

        System.out.print("Salario da segunda pessoa: ");
        salario2 = sc.nextDouble(); // Entrada de variável de ponto flutuante

        System.out.print("Digite uma idade: ");
        idade = sc.nextInt();   // Entrada de variável de número inteiro

        System.out.print("Digite um sexo (F/M): ");
        sexo = sc.next().charAt(0); // Entrada de variável de char

        System.out.println("Nome 1: " + nome1);
        System.out.println("Salario 1: " + String.format("%.2f", salario1));
        System.out.println("Nome 2: " + nome2);
        System.out.println("Salario 2: " + String.format("%.2f", salario2));
        System.out.println("Idade: " + idade);
        System.out.println("Sexo: " + sexo);

        sc.close(); // Encerra o objeto scanner
    }
}
